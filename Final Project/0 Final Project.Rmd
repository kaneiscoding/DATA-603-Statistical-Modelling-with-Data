---
title: "Final Project: Modelling the Impact of COVID-19: Preparing for Future Pandemics"
author: "Paul Croome(30198881), Rodrigo Rosales Alvarez(30148393), Ann Siddiqui(30043081) and Kane Smith (30179486)"
date: "2022-12-14"
output: 
  pdf_document: 
    toc_depth: 4
...
\newpage
# Introduction
The domain of our project covers healthcare-related indicators of the well-being of countries around the world during the coronavirus disease 2019 (COVID-19) pandemic. In particular, we examined data related to the prevalence and severity of the COVID-19 pandemic and the governmental and societal measures taken to reduce the spread of the virus. All the data we used for this project are daily reported between January 2020 and October 2022.  
...
For this reason, the team investigated questions aimed toward quantifying the effects of COVID-19 on public health and toward identifying the healthcare measures that are effective in curbing the pandemic's effects. In particular, we created models to predict the number of COVID-19 cases and deaths that one could expect to observe in a country based on key population metrics and on metrics related to the country's daily response to and experience of COVID-19. Such efforts toward understanding pandemics and their relationships to the countries and populations that they impact may prove essential in helping countries around the world to prepare for future pandemics when they nearly inevitably occur.   
...
The data set we used for this project is provided by Our World in Data (2022) and is licensed for open access under the Creative Commons BY license. We downloaded the data, in CSV format, from the Our World in Data Git Hub repository, here: https://github.com/owid/covid-19-data/tree/master/public/data. The data set consists of diverse information related to the COVID-19 pandemic for countries around the world, including such information as a country’s daily rates of COVID-19 diagnoses, hospitalizations, deaths, and vaccinations (all of which are quantitative data, and available both in counts and in rates). The data set also includes population and health metrics for each of the countries included, such as population, population density, life expectancy, and the prevalence of diabetes in a population. Each row in the data set represents COVID-19 and population/health metric data for a single county on a single date between January 1, 2020 and October 26, 2022.  

The data in our chosen data set was put together by Our World in Data, but includes numerous variables from different sources. For example: the data on each country's COVID-19 vaccinations, tests, cases, and hospitalizations were collected and collated by Our World In Data; data on confirmed COVID-19 cases and deaths were provided by JHU CSSE COVID-19 Data; and data on the governmental policy responses of each country were provided by the Oxford COVID-19 Government Response Tracker. Although we believe there was likely to be under-reporting of COVID-19 cases and deaths in many countries, and we understand that different countries each had different abilities to accurately report the impacts of the pandemic among their populations (due to differences in available resources and funding, for example), we are not aware of any other systematic biases in the reporting of these data from the organizations which themselves provided the data for the data set.    


## Variable Definition

Note: all dependent and independent variables used in our modeling procedures are quantitative variables.  
...
1) Population metrics  
    ...
    - _human_development_index_: Human development index as calculated by [Human Development Reports](https://hdr.undp.org/data-center/human-development-index#/indicies/HDI) (United Nations, 2022).  
    - _population_density_: Population density of the country.  
    - _aged_65_older_: The number of the population per million that is aged 65 years old or older.  
...
3) COVID-19 metrics  
    ...
    - _reproduction_rate_: an estimate of the effective reproduction rate of the COVID-19 virus (the average number of people who will be infected by someone who is infected with the virus).  
...
To answer the questions outlined in our introduction, we created various multiple linear regression models. Our analyses and model construction procedures followed the below structure:  
...
**Analysis**  
    ...
    - Question 2 Multiple Linear Regression Models   
      - Daily COVID-19 Cases Model (Paul Croome)  
        - Full Model Creation  
        - Main Effect Terms   
        - Interaction Terms  
        - Higher Order Terms  
        - Model Assumptions  
        - Best Fitting Model  
      - Daily COVID-19 Deaths Model (Ann Siddiqui)  
        - Full Model Creation  
        - Main Effect Terms   
        - Interaction Terms  
        - Higher Order Terms  
...
**Results**   
    ...
    -Question 1  
      - Average Daily COVID-19 Cases Model  
        - Focal Variable Effects on Average Daily Cases  
        - Prediction of Average Daily Cases  
      - Average Daily COVID-19 Deaths Model  
        - Focal Variable Effects on Average Daily Deaths  
        - Prediction of Average Daily Deaths  
    -Question 2  
      - Daily COVID-19 Cases Model  
        - Focal Variable Effects on Daily Cases  
        - Prediction of Daily Cases  
      - Daily COVID-19 Deaths Model
        - Focal Variable Effects on Daily Deaths  
        - Prediction of Daily Deaths  
...
The above tasks were divided among the team in the following way:  
    - Preparation, including importing libraries, importing the data, cleaning the data, defining variables to be used in analyses, and wrangling the data, was distributed evenly among the four team members.  
    - Analysis of question 1 was divided between Rodrigo and Kane; this included building the MLR models, checking the assumptions, doing any transformations and interpreting the results.  
    - Analysis of question 2 was divided between Paul and Ann, which also included building the MLR models, checking the assumptions, doing any transformations and interpreting the results.  
...
## Data Import
As our data is in CSV format, we used the function **read_csv()** to import our file into a data frame.
```{r}
...
## Data Cleaning 
Several data cleaning procedures were required before we could conduct our analyses and answer our guiding questions, as the data set presented entries for regions of the world that were aggregations of multiple countries (for example, 'Europe' was provided as an aggregation of all countries in the continent of Europe). As we were building models aimed toward predicting COVID-19 cases and deaths on a country level, we needed to remove these aggregations to reduce the noise in our results.  

Many countries and organizations under-reported COVID-19 statistics, such as positive cases and deaths, according to Claire Klobucista from Council of Foreign Relations (Klobucista, 2021). Such under-reporting is detrimental in the real-world because it could cause countries to underestimate the extent to which they must prepare for future pandemics, and it is detrimental to our team's research because it results in inaccurate data for our models. By building our models with innaccurate data, our models would fail to accurately reflect reality. To solve this problem, we removed countries which were present in the bottom fifth percentile of reporting 'new_cases_smoothed_per_million' and 'new_deaths_smoothed_per_million' (that is, the number of daily cases and deaths per million people observed in countries as a result of COVID-19, averaged over 7 days) from the data set. 
```{r}
...
At this stage, the data set was ready to use for answering question 2. However, the following data set cleaning procedures were still required to prepare the data set for answering question 1:  
    - We generated generated a new column called *smokers* that was the average of *female_smokers* and *male_smokers*.  
    - We replaced all the null values in the data set with the last-observed value, because we would be performing an aggregation of all quantitative columns (question 1), and the presence of a single null value would result in all columns being Null.  
    
```{r}
# Creating the column "smokers"
covid_data$smokers <- (covid_data[['male_smokers']] + covid_data[['female_smokers']]) / 2

# Droping columns for continent, iso_code, test_results, and date
...
## Data Preparation 
To create the final data set that will be used to create the *Average Daily Cases Model* and *Average Daily Deaths Model* (question 1), we performed an aggregation function (mean) across the variable country, as we needed to have only one data point per country. This data point would be the mean of every other variable present in the table. We decided to use the mean for this investigation, as it is the best way to aggregate the quantitative variables that we were interested in using (e.g., new_cases, new_deaths, population, stringency_index and more).  
```{r}
...
To answer this question, we built two multiple linear regressions models, one for Average Daily Cases and one for Average Daily Deaths, using the tools we learned during DATA 603.    

The variables we considered for the creation of the models mentioned above, relating to countries' population and health metrics, are:  
...
It is important to state that, for all the statistical tests that we performed throughout this project, we used a significance level of $\alpha = 0.05$.  


### Average Daily COVID-19 Cases Model

We started by defining our full model with the function **lm()**, including all the population- and health-related variables which were available to us and could logically be used to predict New COVID-19 Cases for a specific country. Using the **summary()** function, we were able to see the most important information about this full model. 
```{r}
...
Using the *Step Wise Regression Procedure* to create the best-fit additive model, we used a p-value for inclusion in the model of 0.1 and a p-value for removal from the model of 0.3. In addition, we conducted an individual coefficients t-test with the remaining variables identified by the stepwise regression procedure, using the following hypotheses:  

Hypothesis Testing for Individual Coefficient t tests.  
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i$ = population, aged.65.older, gdp.per.capita  

```{r}
model_cases_stepwise = ols_step_both_p(model_cases_full, pent=0.1, prem=0.3, progress=TRUE)
```

Using the *Step Wise Regression Procedure* to create the best-fit additive model, we found that only 3 variables were significant; for these 3 terms we can reject the null hypothesis for the individual coefficient t-test, meaning that the terms are significant and the coefficients are different from 0 (at a significance level of 0.05). The 3 terms that passed the mentioned tests are: **population **, **gdp_per_capita** and **aged_65_older**.  

The model obtained after performing the *Step Wise Regression Procedure* and individual coefficients t-test is:  
$\widehat{y}_{daily.cases} = - 3863.4429 + 0.00004794 x_{population} + 522.5716 x_{aged.65.older} + 0.0715 x_{gdp.per.capita}$  

At this stage, the $R^2_{adj}$ of our model is: 0.3497, meaning that the proportion of the total variation that is explained by the model, accounting for the number of parameters in the model, is 34.97%.  
```{r}
model_cases_revised = lm(new_cases ~ population + aged_65_older + gdp_per_capita, data=covid_agg)
summary(model_cases_revised)
```

We then performed a global F-test to decide if the model is a better fit for the data than the null model, the mean of the dependent variable, using the following hypotheses:  

$H_{0}: \beta_{population}=\beta_{aged.65.older}=\beta_{gdp.per.capita}=0$  

$H_a:$ at least one $\beta_{i}$ (where i= population, aged.65.older, gdp.per.capita) does not equal 0.  
...
From the output of our global F-test, we obtained a p-value of 0.00000000000000022, which is less than our $\alpha$ value of 0.05. Thus, we can reject our null hypothesis that all of our regression coefficients are equal to 0 and can infer with a significance level of 0.05 that our model has at least one significant predictor.  


Using interactions terms can be a powerful way to increase the effectiveness of our model, as sometimes the effect of an independent variable on a dependent variable, in our case new COVID-19 cases, is not constant over all of the values of other independent variables.   

To test for interaction terms among the main effects of our model, we conducted individual coefficients t-tests for all of the interaction terms, using the following hypotheses:  

$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  

where $i$ is any pairwise combination of population, aged.65.older, and gdp.per.capita. 
...
```
P-value < $\alpha$ for the interactions terms for **population:gdp_per_capita** and **aged_65_older:gdp_per_capita**; therefore, we rejected our null hypothesis for interaction terms, inferring at the 0.05 significance level that they should be included in the model.  

Below is the updated interaction model with all the significant terms.  
```{r}
...
As the relationship between the independent variables and the dependent variable is sometimes not linear, but is instead curvilinear, we next checked for higher-order terms. To do this, we first checked to see the correlations between our independent variables and the dependent variable to determine which variables we needed to check for higher-order terms.  
```{r}
...
From the pairwise plots above, we can see a curvilinear relationship between population and new_cases. As this relationship is the one with the highest correlation coefficient, we included a higher order term to try to improve our model.  
...
```
p-value < $\alpha$ for the term population^2; therefore, we failed to reject the null hypothesis for the population-squared, and did not include it in the model.  

Our Final Best Fit Model is:  
$\widehat{y}_{daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$  
Our model has an $R^2_{adj}$ of 0.6551.  
...
From the output of our test, we obtained a p-value of 0.00000000000000022, which is less than our $\alpha$ of 0.05. This means we can reject our null hypothesis that our sample data is significantly normally distributed and conclude (at the 0.05 significance level) that our sample data is not normally distributed. This means that there is a problem with the normality assumption.  
...
From the output of our test, we ontain a p-value of 0.0008982 which is less than 0.05. This means that we reject the null hypothesis and conclude (at the 0.05 significance level) that our model is heteroscedastic. This means that there is a problem with the homoscedasticity assumption.   
...
```
Looking at the plot above, our model is likely linear. However, it is inconclusive, as simply viewing a graph involves subjectiv judgment.  
...
plot(model_cases_interactions_2, which=5) + title('Figure 4: Cook´s Distance Plot for Average Daily COVID-19 Cases
                                                  ')
```
From our output, we do not have any outliers in our data set that affect our results; therefore, there is no problem in our model with outliers, and no further action needed.  
...
```
Looking at the plot above, it appears that the residuals are evenly distributed, meaning that the residuals are independent from each other. However, it is not conclusive as we are again making subjective judgments from visual inspection of a graph.  

#### Multicolinearity
The Multicollinearity assumption checks if two or more terms in our model provide redundant information about the independent variable. We tested for multicollinearity in our model by using the Variance Inflation Factor (VIF).  
```{r}
imcdiag(model_cases_revised, method='VIF')
```
A value in VIF between 1 and 5 suggest that there is a moderate multicollinearity, but it is not severe enough to warrant corrective measures. Therefore, we can conclude that our model passed this assumption.  



#### Final Model
As shown above, our final model did not pass the assumptions of homoscedasticity or normality. Our model did, however, pass the assumptions of multicolinearity, linearity and independent errors. When attempting a Box Cox transformation, we got an error saying that *response variable must be positive*. Researching this online, we found that we could have used the Yeo-Johnson transformation; however, this is beyond the scope of this course, so we did not do it.  

The final model to predict the average new daily COVID-19 cases for a specific country using is:  
$\widehat{y}_{daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$  
Our model has an $R^2_{adj}$ of 0.6551. Our RSME is 6208.4082.  
...
print(paste0('RSME: ', sigma(model_cases_final)))
```
...
For the Average Daily Deaths model, we followed the same procedure as completed for the Average Daily Cases model. We began by defining our full model, with **lm()**, including all the population- and health-related variables that make sense to predict New COVID-19 Deaths for a specific country. Using the **summary()** function we are able to see the most important information about our model.
```{r}
...
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i$ = population, human.development.index  
```{r}
model_deaths_stepwise = ols_step_both_p(model_deaths_full, pent=0.1, prem=0.3, progress=TRUE)
```

Using the *Step Wise Regression Procedure* to create the best-fit additive model, we found out that only 2 variables were significant; for these 2 terms, we could also reject the null hypothesis for the individual coefficient t-test, meaning that the terms are significant and the coefficients are different than 0 (at a significance level of 0.05). The 2 terms that passed the mentioned test are: **population** and **human_development_index**.  
...
$R^2_{adj}$ of our model is: 0.6635, meaning that the proportion of the total variation that is explained by the model is 66.35%.  
```{r}
...
From the output of our global F-test, we get a p-value of 0.000000000000001914  which is less than our $\alpha$ value of 0.05. Thus, we can reject our null hypothesis that all of our regression coefficients are equal to 0 and can conclude with a significance level of 0.05 that our model has at least one significant predictor.  

Using interactions terms can be a powerful way to increase the effectiveness of our model as the effect of an independent variable on a dependent variable, in our case new COVID-19 deaths, is sometimes not constant over all of the values of the other independent variables. With population among our significant predictor terms, we decided to include interactions terms, as all other variables could logically change, depending on the total population of a country.  

Hypothesis Testing for Individual Coefficient t tests.  
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i$ = population:human_development_index  
...
```
As p-value < $\alpha$ for the interaction term, we rejected our null hypothesis for the interaction term **population:human_development_index** and included it in the model.  
...
Our $R^2_{adj}$ is now 0.5141, which is almost double the $R^2_{adj}$ of the previous additive model.  


We then checked for potential significant higher-order terms to see if we could improve our $R^2_{adj}$ further, as some of the relationships with the dependent variable could be curvilinear instead of linear. To check these relationships, we used the ggpairs plot of correlations between the independent variables and the dependent variables.  
```{r}
...
We can observe in the above plots a curvilinear relationship between population and new_deaths; for this reason, we decided to check whether a square term of population improves the model.  
...
As p-value > $\alpha$, we failed reject the null hypothesis for the squared term, population. Thus, we did not include it in the model.  
...
From the output of our test, we obtained a p-value of 0.00000000000000022, which is lower than 0.05. This means we reject the null hypothesis and conclude with a significance level of 0.05 that our sample data is not normally distributed. This means that there is a problem with the normality assumption.  
...
```
Looking at the plot above, our model is likely linear. However, it is not conclusive as judging a graph by visual inspection can be subjective.  
...
From our output, there appear to be two data points, row 88 and 207, with Cook´s Distance over 1. Therefore, we deleted them from the data set and re-fit the model afterwards to see if the model improved.  
```{r}
...
#### Multicolinearity
The multicollinearity assumption checks if two or more terms in our model provide redundant information about the independent variable. We used the Variance Inflation Factor (VIF) to determine the presence of multicollinearity in our model.  
```{r}
imcdiag(model_deaths_revised, method='VIF')
```
A value of VIF close to or below 1 means that there is no multicollinearity. Thus, we were able to conclude that there was no multicollinearity present, and our model passed this assumption.  
...
#### Final Model
As exhibited above, our final model passed the assumptions of multicolinearity, homoscedasticity, independent errors, and linearity. This model only failed to satisfy the normality assumption. When attepting a Box Cox transformation, we received an error saying that *response variable must be positive*. From looking online, we determined that we could have used the Yeo-Johnson transformation; however, it is outside of the scope of the course, and so we did not use it.  

We will now re-fit our final model with the dataset without outliers. 
...
Evidently, after building the model without outliers, the $R^2_{adj}$ dropped drastically. Thus, we decided to leave the outliers in the model.  

Our final model for average new daily deaths due to COVID-19 is:  
$\widehat{y}_{daily.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$  
More important our $R^2_{adj}$ is now 0.5141. Our RMSE = 87.5619.  

We decided to declare the previously constructed model as the final model for average daily deaths for question 1, despite its violation of one assumption. It is important to note, therefore, that the results of the model are not necessarily trustworthy.  
```{r}
model_deaths_final = lm(new_deaths ~ (population + human_development_index)^2, data=covid_agg)
summary(model_deaths_final)
print(paste0('RSME: ', sigma(model_deaths_final)))
```
...
To answer this guiding question, we will use the tools we learned in DATA 603 to develop two multiple linear regression models: one for Daily Cases (i.e., daily COVID-19 cases reported in a country) and one for Daily Deaths (i.e., daily deaths due to COVID-19 reported in a country). For either model, the independent/predictor variables which we consider for inclusion in the model are:  
...
To determine whether any interaction terms should be included in the model, we created a full interaction model and then conducted individual coefficients t-tests with each new interaction term, using the same hypotheses as above (and at a significance level of $\alpha = 0.05$):  
  ...
  where $\beta_i$ represents the coefficients for each of the interaction terms between independent variables included in the model.  
...
P-value > $\alpha$ for the interaction terms positive_rate:stringency_index and reproduction_rate:positive_rate; therefore, we fail to reject the null hypothesis for these terms and infer that their coefficients are not different from 0, at a 5% significance level.  

For all other interaction terms, p-value < $\alpha$. Therefore, the null hypothesis was rejected these terms, and they were all included in the model going forward.  
...
At this stage, the model had an RMSE of 3582 (far lower than the RMSE of 20560 when only main effects were included) and an $R^2_{adj}$ of 0.9918 (far higher than the additive model's $R^2_{adj}$ of 0.7301).  
...
We once again used a significance level of $\alpha = 0.05$.

```{r}
bptest(gq2_cases_intermod_reduced)
```

From the output of our test, we obtained a p-value of 0.00000000000000022, which is less than 0.05. P-value < $\alpha$; thus, we rejected the null hypothesis that there is homoscedasticity and concluded with a significance level of 0.05 that our model is not homoscedastic (i.e., we can infer that heteroscedasticity is present). This means that there is a problem with the homoscedasticity assumption.  
...
We obtained no rows where new cases were less than 0, so we continued with the Box-Cox transformation.
...
The best lambda value is 0.3939394; this is the lambda value which we used to conduct the following Box-Cox transformation:  
...
After conducting a Box-Cox transformation, our model had an $R^2_adj$ of 0.3264, which is far lower than the previous $R^2_adj$ of 0.9918. For this reason, we decided to declare the previously constructed model as the final model for daily new cases, despite its violation of the assumptions of homoscedasticity and normality (and potential failure to satisfy the assumptions of independence of errors and of linearity).  

It is important to note that, due to the model's violations of the assumptions ofhomoscedasticity and normality, the results of the model are not necessarily trustworthy.  
...
To determine whether any interaction terms should be included in the model, we created a full interaction model and then conducted individual coefficients t-tests with each new interaction term, using the same hypotheses as above (and at a significance level of $\alpha = 0.05$):  
  ...
  where $\beta_i$ represents the coefficients for each of the interaction terms between the independent variables.  
...
Finally, p-value < $\alpha$ for all interaction terms; therefore, we rejected the null hypothesis for all of these terms and inferred at the 5% significance level that their coefficients are not equal to zero. In other words, all of the terms in the model created above should be included in the model going forward.  

At this stage, the model has an RMSE of 85.25 (much lower than the RMSE of 115.1 when only main effects were included) and an $R^2_{adj}$ of 0.9197 (notably higher than the additive model's $R^2_{adj}$ of 0.8537).  
...
The results above indicate that the addition of a $X_{positive.rate}^2$ term increased the $R^2_{adj}$ by only 0.0002 from the previous first-order interaction model, and decreased the RMSE by only 0.05. Therefore, the model which includes the higher-order term fits the data only marginally better, and not significantly enough to warrant inclusion in the final model.  
...
From the output of our test, we obtained a p-value of 0.00000000000000022, which is less than 0.05. P-value < $\alpha$; therefore, we can reject the null hypothesis that our sample data is significantly normally distributed and conclude with a significance level of 0.05 that our sample data is not normally distributed. This means that there is a problem with the normality assumption. This result is reflected in the Q-Q plot as well, in which one can observe that the residuals deviate increasingly far from the diagonal line of normality as values fall further from 0.  
...
We got no rows where new deaths were less than 0, so we continued with the Box-Cox transformation. However, when attempting to conduct a Box-Cox transformation, we received an error saying "response variable must be positive", despite having confirmed that the response variable is never less than 0. After researching this issue, we found that we could have used the Yeo-Johnson transformation; however, this is beyond the scope of this course, so we decided not to do it.  

For this reason, we decided to declare the previously constructed model as the final model for daily new deaths, despite its violation of the assumptions of homoscedasticity and normality. It is important to note, therefore, that the results of the model are not necessarily trustworthy.  

The final Daily Deaths Model is:  
...
After performing a stepwise regression, we found that the following terms were significant predictors:  

    - population   
    - aged_65_older   
    - gdp_per_capita  

We additionally conducted an individual coefficients t-test with the terms mentioned above and rejected the null hypothesis for each one, meaning that they were significant and warranted inclusion in our model.  
...
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i =$ any pairwise interaction between population, aged_65_older, gdp_per_capita  

We rejected the null hypothesis for *population:gdp_per_capita* and *aged_65_older:gdp_per_capita*, so we included them in the model.  
...
We tested higher-order terms with the highest correlation to the dependent variable, finding that the independent variable of population warranted investigation. We used an individual coefficient t-test to determine the significance of a squared term for population and failed to reject the null hypothesis, meaning that the higher-order term should not be included in our model.  
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i =$ population^2  

We checked the following regression assumptions for our model:  

*Normality*  
Using the Shaphiro-Wilk test, we rejected our null hypothesis, meaning that our data is not significantly normally distributed.  
The Normality Assumption was not met.  
**Null hypothesis:** the sample data are significantly normally distributed.  
**Alternative hypothesis:** the sample data are not significantly normally distributed.  

*Homoscedasticity*  
Using the Breusch-Pagan test, we rejected our null hypothesis, meaning that heteroscedasticity is present.  
The Homoscedasticity Assumption was not met.  
**Null hypothesis:** heteroscedasticity is not present ($H_0: \sigma^2_1 = \sigma^2_2 =... = \sigma^2_n$).  
**Alternative hypothesis:** heteroscedasticity is present ($H_a:$ at least one $\sigma^2_i$ is different from the others).  
...
$\widehat{y}_{new.daily.cases} = - 317.9720 - 0.0000004255 x_{population} + 69.5871 x_{aged.65.older} - 0.0702 x_{gdp.per.capita} + 0.000000005053 x_{population} x_{gdp.per.capita} + 0.0100 x_{aged.65.older} x_{gdp.per.capita}$  
$R^2_{adj}$ = 0.6551. RSME = 6,208.4082.  

Despite not passing the normality and homoscedasticity assumptions, we decided to use the mentioned model as our final model for prediction purposes. However, we understand that results may not be trustworthy.  


#### Focal Variable Effects on Average Daily Cases
The coefficient estimates for the focal variables in the final model above imply the following:  

- *population*: for every increase of 1.0 in the amount of people that live in a country, the number of average daily COVID-19 cases will increase by -317.9720 + (0.000000005053 * gdp_per_capita).  
- *aged_65_older*: for every increase of 1% of the proportion of people over 65 years old in a country, the number of daily COVID-19 cases will increase by -248.3849 + (0.0100 * gdp_per_capita).  
- *gdp_per_capita*: for every 1 dollar increased in the gross per capita of a country, the  number of daily COVID-19 cases will increase by -318.0422 + (0.000000005053 * population) + (0.0100 * aged_65_older).  


#### Prediction of Average Daily Cases
The purpose of building a model is to have the ability to predict unknown scenarios. Using the model we created we predicted the average daily COVID-19 cases for a hypothetical country whose population is defined by the mean values of all our predictor terms and for a hypothetical country whose whose population is defined by the meian values of all our predictor terms.  
```{r}
...
After performing a stepwise regression method, we ended up with the following terms:  

    - population  
    - human_development_index  

We rejected the null hypothesis for the terms mentioned above in an individual coefficients t-test as well, meaning that they were significant and warranted inclusion them in our model.  
...
Then, we checked for interactions terms and performed an individual coefficient t-test to evaluate which interactions terms to include in the model.  
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i =$ interaction term, population:population_density  

We rejected the null hypothesis for *population:human_development_index*, so we include it in the model.  
$\widehat{y}_{daily.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$  
$R^2_{adj}$ = 0.5141.  

We tested higher-order terms with the highest correlation to the dependent variable, and found that the independent variable of population was most correlated. Therefore, we conducted an individual coefficient t-test for this term and failed to reject the null hypothesis, meaning that the higher-order term should not be included in our model.  
$H_0: \beta_i = 0$  
$H_a: \beta_i \neq 0$  
$i =$ population^2  

Finally, we checked the following regression assumptions for the model:  
...
*Multicolinearity*  
Using the VIF method to test for multicolinearity, we observed that it was not present in our model, meaning that the multicolinearity assumption was met.  
    

Our final model to predict average daily deaths of COVID-19 according to population-related metrics is:  
$\widehat{y}_{daily.deaths} = 0.8449 - 0.000004976 x_{population} + 10.9375 x_{human.development.index} + 0.000008337 x_{population} x_{human.development.index}$  
$R^2_{adj}$ = 0.5141. RMSE = 87.5619.  

Despite not passing the normality assumption, we decided to use the aforementioned model as our final model for prediction purposes, though we understand that results may not be trustworthy.  


#### Focal Variable Effects on Average Daily Deaths
The coefficient estimates for the focal variables in the final model above imply the following:  

- *population*: for every increase of 1.0 in the amount of people that live in a country, the number of average daily deaths due to COVID-19 will increase by 0.8449 + (0.000008337 * human_development_increase).  
- *human_development_index*: for every increase of 1% in the Human Development Index, the number of average daily deaths due to COVID-19 will increase by 0.954275 + (0.00000008337 * population).  


#### Prediction of Average Daily Deaths
The purpose of building a model is to have the ability to predict unknown scenarios. Using the model we created we predicted the average daily COVID-19 deaths for a hypothetical country whose population is defined by the mean values of all our predictor terms and for a hypothetical country whose whose population is defined by the meian values of all our predictor terms.  
```{r}
...
Before declaring the final model, we checked the necessary regression assumptions for the model:  

    - For the normality assumption, we conducted a Shapiro-Wilk test, which resulted in our rejection of the null hypothesis and concluding that the model does not meet the assumption of normality.  

    - To check for homoscedasticity in the residuals, we used a Breusch-Pagan test, which also resulted in a rejection of the null hypothesis and conclusion that the model exhibits heteroscedasticity.  

    - For the assumptions of linearity and independence, we plotted the residuals of the model against the fitted values, finding that there was potentially a funnel-pattern in the data points, but no pattern obvious enough to declare a failure of the assumptions. For this reason, we concluded that these assumptions are satisfied by the model.  

    - Lastly, we checked for outliers in the model by seeing if any data points had a Cook's Distance greater than 1.0, as visualized in a plot of residuals against leverage values. From this investigation, we found no outliers which warranted removal from the data set (and only one point had a Cook's Distance greater than 0.5).  

Therefore, we determined that the model failed both the normality and homoscedasticity assumptions, while the linearity, independence, outlier, and multicollinearity assumptions were all satisfied.   

To rectify the problems with normaltiy and homoscedasticity, we attempted a Box-Cox transformation of the response variable, calculating the best lambda for this procedure (0.3939394) and applying it to the new_cases_smoothed variable when re-fitting the model. The output of this transformation, however, was a model with a remarkably lower $R^2_{adj}$ of only 0.3264.  

As a result, we determined that the best-fit model we would be able to put forth for daily COVID-19 cases, based on daily-reported COVID-19-related data (and understanding that the results of the model may not be trustworthy), was:  
...
The coefficient estimates for the focal variables in the final model above imply the following:  
  ...
  - *reproduction_rate*: for every increase of 1.0 in the daily estimate of the reproduction rate of COVID-19, the number of predicted new daily cases (averaged over 7 days) changes by -177.4731 - (0.0028 * new_tests_smoothed)  - (0.0114 * new_vaccinations_smoothed) + 0.0505 (new_people_vaccinated_smoothed) + (58.5161 * stringency_index) - (1.4194 *  hosp_patients).  
  - *new_tests_smoothed*: for every increase of 1 in the number of daily COVID-19 tests conducted (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by 0.0103 - (0.0028 * reproduction_rate) + (0.9823 * positive_rate) - (0.00000001809 * new_vaccinations_smoothed) + (0.000000008441 * new_people_vaccinated_smoothed) - (0.0001008 * stringency_index) + (0.0000001965 * hosp_patients).  
  - *positive_rate*: for every increase of 1% in the share of COVID-19 tests which find a positive case (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by 7.824864 + (0.009823 * new_tests_smoothed) - (0.0005656 * new_vaccinations_smoothed) - (0.0003826 * new_people_vaccinated_smoothed) + (0.020004 * hosp_patients).  
  - *new_vaccinations_smoothed*: for every increase of 1.0 in the number of daily COVID-19 vaccination doses administered (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by 0.0443 - (0.0114 * reproduction_rate) + (0.00000001809 * new_tests_smoothed) - (0.05656 * positive_rate) + (0.000000005571 * new_people_vaccinated_smoothed) - (0.0004916 * stringency_index) + (0.0000005381 * hosp_patients).  
  - *new_people_vaccinated_smoothed*: for every increase of 1.0 in the daily number of people receiving their first dose of a COVID-19 vaccination (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by -0.1002 + (0.0505 * reproduction_rate) + (0.000000008441 * new_tests_smoothed) - (0.03826 * positive_rate) + (0.000000005571 * new_vaccinations_smoothed) + (0.0009311 * stringency_index) - (0.0000007765 * hosp_patients).  
  - *stringency_index*: for every increase of 1/100 in the daily reported stringency index regarding a country's governmental regulations toward COVID-19, the number of predicted new daily cases (averaged over 7 days) changes by -35.6258 + (58.5161 * reproduction_rate) - (0.0001008 * new_tests_smoothed) - (0.0004916 * new_vaccinations_smoothed) + (0.0009311 * new_people_vaccinated_smoothed) - (0.002075 * hosp_patients).  
    - *hosp_patients*: for every increase of 1 COVID-19 patient in hospital on a given day, the number of predicted new daily cases (averaged over 7 days) changes by 1.2017 - (1.4194 * reproduction_rate) + (0.0000001965 * new_tests_smoothed) + (2.0004 * positive_rate) + (0.0000005381 * new_vaccinations_smoothed) - (0.0000007765 * new_people_vaccinated_smoothed) - (0.002075 * stringency_index).  

#### Prediction of Daily Cases

Next, we used our Daily Cases Model to determine the number of predicted deaths experienced for a country in a single day based on certain values of each of the predictor variables included in the model. For the values of prediction, we used the mean and median values of all of the predictor variables included in the model.  
```{r}
...
Before declaring the final model, we checked the necessary regression assumptions for the model:  
    ...
    - For the normality assumption, we conducted a Shapiro-Wilk test, which resulted in our rejection of the null hypothesis and concluding that the model does not meet the assumption of normality.  
    - To check for homoscedasticity in the residuals, we used a Breusch-Pagan test, which also resulted in a rejection of the null hypothesis and conclusion that the model exhibits heteroscedasticity.  
    - For the assumptions of linearity and independence, we plotted the residuals of the model against the fitted values, finding that there was no clear pattern in the data points. For this reason, we concluded that these assumptions are satisfied by the model.  
    - Lastly, we checked for outliers in the model by seeing if any data points had a Cook's Distance greater than 1.0 (as well as none above 0.5), as visualized in a plot of residuals against leverage values. From this investigation, we found no outliers which warranted removal from the data set.  

Therefore, we determined that the model failed both the normality and homoscedasticity assumptions, while the linearity, independence, and outlier assumptions were all satisfied.   

To rectify the problems with normality and homoscedasticity, we attempted a Box-Cox transformation of the response variable, but received only a warning from R that the response variable was not always positive (despite checking this requirement previously). Because the workaround for this issue goes beyond the scope of this course, and we already had a model which explained over 90% of the variance of the data, accounting for the number of variables in the model, we decided that we would declare our final model as the best-fit interaction model.  

The best-fit model for daily COVID-19 deaths, based on daily-reported COVID-19-related data, was:  
...
The coefficient estimates for the focal variables in the final model above imply the following:  
  ...
  - *reproduction_rate*: for every increase of 1.0 in the daily estimate of the reproduction rate of COVID-19 (that is, the average number of people who will be infected by someone who is infected with the virus), the number of predicted new daily cases (averaged over 7 days) changes by -35.3352 - (0.0002561 * new_tests_smoothed)  + (0.0005209 * new_vaccinations_smoothed) -(0.0006226 * new_people_vaccinated_smoothed) + (0.4797 * stringency_index) - (0.09270 *  icu_patients).  
  - *new_tests_smoothed*: for every increase of 1 in the number of daily COVID-19 tests conducted (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by 0.0004054 - (0.0002561 * reproduction_rate) + (0.0007177 * positive_rate) - (0.00000000006414 * new_vaccinations_smoothed) + (0.0000000008364 * new_people_vaccinated_smoothed) - (0.000001561 * stringency_index) + (0.000000008622 * icu_patients).  
  - *positive_rate*: for every increase of 1% in the share of COVID-19 tests which find a positive case (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by -100.6623 + (0.0007177 * new_tests_smoothed) - (0.003121 * new_people_vaccinated_smoothed) + (3.0156 * stringency_index) - (0.1275 * icu_patients).  
  - *new_vaccinations_smoothed*: for every increase of 1.0 in the number of daily COVID-19 vaccination doses administered (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by 0.000004938 + (0.0005209 * reproduction_rate) - (0.00000000006414 * new_tests_smoothed) - (0.0000000001269 * new_people_vaccinated_smoothed) - (0.00001073 * stringency_index) + (0.00000001979 * icu_patients).  
  - *new_people_vaccinated_smoothed*: for every increase of 1.0 in the daily number of people receiving their first dose of a COVID-19 vaccination (averaged over 7 days), the number of predicted new daily cases (averaged over 7 days) changes by -0.001109 - (0.0006226 * reproduction_rate) + (0.0000000008364 * new_tests_smoothed) + (0.003121 * positive_rate) - (0.0000000001269 * new_vaccinations_smoothed) + (0.00002719 * stringency_index) - (0.00000005890 * icu_patients).  
  - *stringency_index*: for every increase of 1/100 in the daily reported stringency index regarding a country's governmental regulations toward COVID-19, the number of predicted new daily cases (averaged over 7 days) changes by -0.8754 + (0.4797 * reproduction_rate) - (0.000001561 * new_tests_smoothed) + (3.0156 * positive_rate) - (0.00001073 * new_vaccinations_smoothed) + (0.00002719 * new_people_vaccinated_smoothed) + (0.0008637 * icu_patients).  
    - *icu_patients*: for every increase of 1 COVID-19 patient in hospital on a given day, the number of predicted new daily cases (averaged over 7 days) changes by 0.1136 - (0.09270 * reproduction_rate) + (0.000000008622 * new_tests_smoothed) - (0.1275 * positive_rate) + (0.00000001979 * new_vaccinations_smoothed) - (0.00000005890 * new_people_vaccinated_smoothed) + (0.0008637 * stringency_index).  

#### Prediction of Daily Deaths

Next, we used our Daily Death Model to determine the number of predicted deaths experienced for a country in a single day based on certain values of each of the predictor variables included in the model. For the values of prediction, we used the mean and meidan values of all of the predictor variables included in the model.  
```{r}
# Creating data frame of the values used for predicting daily cases
...
After performing the necessary statistical analyses for our linear models, the team found (toward our first research question) that the most important population- and health-related metrics for predicting the average number of daily COVID-19 deaths and cases include: population, elderly population (65 years of age or older), GDP per capita, and HDI. Toward our second research question, the team concluded that COVID-19 vaccinations, government regulations, hospitalizations due to COVID-19, and COVID-19 tests are significant predictors of daily COVID-19 deaths and cases.  

## Research Question 1
Our model built for predicting average new daily COVID-19 cases did not pass the homoscedasticity and normality assumptions, though it did pass the multicollinearity, linearity, and independent error assumptions. This model yielded an $R^2_{adj}$ value of 65.55%. Considering these facts together, the model for average daily cases from research question 1 is reasonably well-fitted to the data, but is not trustworthy for drawing conclusions.  

When assessing our second model from research question 1, average daily deaths from COVID-19, the only assumption the model did not pass is the normality assumption. The model passed the assumptions of homoscedasticity, multicollinearity, independent errors and linearity assumptions, and had an $R^2_{adj}$ value of 51.41%. Like the cases model, this model explains a reasonable amount of the variance in the data (when considering that this is real-world data). However, its failure of the normality assumption means that the results of the model are not necessarily trustworthy.  

## Research Question 2
For the first model in research question 2, the team created a model to predict daily COVID-19 cases in a country based on daily-reported COVID-19-related data. This model failed the normality and homoscedasticity assumption, but passed the assumptions of linearity, independent errors, and multicollinearity, while yielding an $R^2_{adj}$ value of 99.18%. Though the $R^2_{adj}$ value indicates a model which fits the data remarkably well, its failure of two assumptions means that it produces untrustworthy results.  

Additionally in research question 2,  the team created a model to predict daily COVID-19 deaths based on daily-reported COVID-19-related data. This model did not pass the homoscedasticity or normality assumptions, but did satisfy the assumptions of linearity, multicollinearity, and independent errors. This model had a very high $R^2_{adj}$ value as well, at about 91.97%. However, as the $R^2_{adj}$ is not the only thing used to deem a model’s success, the team concludes that this model is again untrustworthy to be used to predict data accurate to reality, due to its failure of two of the assumptions.  

## General Conclusions
In order for more conclusive and trustworthy models to be built, there are a few things that can be done. First, in order to address the errors the team received when performing the Box Cox transformation method that states the response variable must be positive, the team could use the Yeo-Johnson transformation method to yield a response variable that would be positive.  

Next, data sets that could be combined with the current data set in the future in order to better assess the factors at play when it comes to quantifying the severity of the COVID-19 pandemic include:  

- Stock Index Data: Are countries with a higher performing stock indices better-equipped for dealing with a global pandemic? Does the performance of the stock market in a specific country indicate any correlations with how COVID-19 was handled in that country?  
- Education Metrics: Do countries that receive better education fare better when their government and healthcare systems respond to COVID-19 due to the population believing in scientific facts?  
- International Travel and Proximity Data: How does the degree of interconnection between countries (e.g., countries' proximities to other countries, their shared borders, etc.) and the amount of travel connections between countries (e.g., number of flights, train connections, buses traveling between countries, etc.) influence the spread and impact of a pandemic in different countries?  

Ultimately, through the creation of various multiple linear regression models to uncover data-driven answers to our 2 research questions, the team was able to yield interesting and important results. Despite the models being potentially unreliable for use in making robust and indisputable conclusions, the team answered their research questions to the best of their ability from their training in DATA 603. We discovered that the population, elderly population, GDP per capita, and HDI are the population metrics which are most strongly related to the prevalence and severity of COVID-19. Moreover, the team found that COVID-19 vaccinations, government regulations, and COVID-19 testing were all highly significant predictors of the severity and prevalence of COVID-19 observed in countries on a daily basis.    



\newpage
# References
Klobucista, C. (2021). By How Much Are Countries Underreporting COVID-19 Cases and Deaths?. Council of Foreign Relations. https://www.cfr.org/in-brief/how-much-are-countries-underreporting-covid-19-cases-and-deaths  
Our World in Data. (2022a). COVID-19 Data Explorer. Our World in Data. https://ourworldindata.org/explorers/coronavirus-data-explorer  
Our World in Data. (2022b). Data pipeline. Data pipeline - COVID-19 dataset by Our World in Data. Retrieved December 12, 2022, from https://docs.owid.io/projects/covid/en/latest/data-pipeline.html#overview  
United Nations. (2022). Human Development Index. United Nations Development Programme; United Nations. https://hdr.undp.org/data-center/human-development-index#/indicies/HDI  
